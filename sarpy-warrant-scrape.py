...
while (count < numwarrants):
    ...
    almostthere = namething.replace('Name:','').replace('DOB:','|').replace('\t','').replace('\n','').replace('\xc2','').replace('  ',' ').replace('  ',' ').replace('&nbsp','').replace('\xa0','').replace('  ','|', 1).replace('  ',' ')
